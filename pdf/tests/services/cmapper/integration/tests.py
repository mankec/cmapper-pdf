...
class CmapperIntegrationTestCase(TestCase):
    ...
    def test_show_pdf_page_in_word_blocks(self):
        ...
        self.assertEqual(
            expected_blocks_len, len(first_page.get_text(Cmapper.TEXT_FORMAT_DICT)["blocks"])
        )
        self.assertEqual(
            expected_blocks_len, len(second_page.get_text(Cmapper.TEXT_FORMAT_DICT)["blocks"])
        )
        ...
        for sentence in first_page_blocks:
            for word in sentence.split(" "):
                word_url = reverse("pdf:word", kwargs={"pno": Cmapper.DEFAULT_PNO, "word": word})
